...
class ArticleController extends Controller
{
    ...
    public function createAction()
    {
        ...
        try {
            ...
            if ($this->request->isPost()) {
                ...
                $data['CategoryId'] = Category::NOTICE;
                $data['Status'] = Article::STATUS_UN;
                if (empty($data['Author']) || !isset($data['Author'])) {
            ...
            //接收方式
            if (!is_array($data['Way']) || !count($data['Way'])) {
                $exception->add('Way', '接收方不能为空');
                throw $exception;
            }
            $data['AcceptOrganization'] = array_sum($data['Way']);
    ...
    public function listAction()
    {
        ...
        try {
            ...
            $query->where('A.OrganizationId=' . Organization::PEACH);
    ...
    {
        $query = $this->modelsManager->createBuilder()
            ->columns('L.Id,L.ArticleId,L.StaffId,L.Operated,L.Created,S.Name')
            ->addFrom(StaffNoticeLog::class, 'L')
            ->join(Staff::class, 'S.Id=L.StaffId', 'S', 'left')
            ->where('L.ArticleId=:ArticleId:', ['ArticleId' => $this->request->get('Id', 'int')])
            ->getQuery()
            ->execute();
